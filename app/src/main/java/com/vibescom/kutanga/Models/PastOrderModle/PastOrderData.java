...
public class PastOrderData extends BaseModel implements Serializable {
    ...
    public CopyOnWriteArrayList<OrderDetailsModel> getInTheSport() {
        return inTheSport;
    }

    public void setInTheSport(CopyOnWriteArrayList<OrderDetailsModel> inTheSport) {
        this.inTheSport = inTheSport;
    }

    private CopyOnWriteArrayList<OrderDetailsModel> inTheSport;

    public PastOrderData(JSONObject jsonObject){
        ...
        this.address=getValue(jsonObject,kFormatedaddress,String.class);
        this.userAddress=getValue(jsonObject,kuserAddress,String.class);
        ...
        if(jsonObject.has(kOrderDetails)){
            try {
                this.inTheSport = handleMarketPlace(jsonObject.getJSONArray(kOrderDetails));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
    ...
    private CopyOnWriteArrayList<OrderDetailsModel> handleMarketPlace(JSONArray jsonArray) throws JSONException {
        CopyOnWriteArrayList<OrderDetailsModel> marketPlaces = new CopyOnWriteArrayList<>();
        for (int i = 0; i < jsonArray.length(); i++) {
            marketPlaces.add(new OrderDetailsModel(jsonArray.getJSONObject(i)));
        }
        return marketPlaces;
    }
