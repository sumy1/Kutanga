...
public class TopPicModel {
    ...
    public CopyOnWriteArrayList<TopPicData> getTopPicDta() {
        return topPicDta;
    }

    public void setTopPicDta(CopyOnWriteArrayList<TopPicData> topPicDta) {
        this.topPicDta = topPicDta;
    }

    private CopyOnWriteArrayList<TopPicData> topPicDta;


    public TopPicModel(JSONObject jsonObject) {
        ...
        if(jsonObject.has(kOfferdata)){
            try {
                this.topPicDta = handleMarketPlace(jsonObject.getJSONArray(kOfferdata));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
    ...
    private CopyOnWriteArrayList<TopPicData> handleMarketPlace(JSONArray jsonArray) throws JSONException {
        CopyOnWriteArrayList<TopPicData> marketPlaces = new CopyOnWriteArrayList<>();
        for (int i = 0; i < jsonArray.length(); i++) {
            marketPlaces.add(new TopPicData(jsonArray.getJSONObject(i)));
        }
        return marketPlaces;
    }
}
