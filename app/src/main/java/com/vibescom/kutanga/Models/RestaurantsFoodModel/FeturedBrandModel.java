...
public class FeturedBrandModel {
 ...
 private String ImagePath;
    ...
    public CopyOnWriteArrayList<FeaturedBranddataModel> getFetureddta() {
        return fetureddta;
    }

    public void setFetureddta(CopyOnWriteArrayList<FeaturedBranddataModel> fetureddta) {
        this.fetureddta = fetureddta;
    }

    private CopyOnWriteArrayList<FeaturedBranddataModel> fetureddta;



    public FeturedBrandModel(JSONObject jsonObject){
        ...
        if(jsonObject.has(kOfferdata)){
            try {
                this.fetureddta = handleMarketPlace(jsonObject.getJSONArray(kOfferdata));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
    ...
    private CopyOnWriteArrayList<FeaturedBranddataModel> handleMarketPlace(JSONArray jsonArray) throws JSONException {
        CopyOnWriteArrayList<FeaturedBranddataModel> marketPlaces = new CopyOnWriteArrayList<>();
        for (int i = 0; i < jsonArray.length(); i++) {
            marketPlaces.add(new FeaturedBranddataModel(jsonArray.getJSONObject(i)));
        }
        return marketPlaces;
    }
}
