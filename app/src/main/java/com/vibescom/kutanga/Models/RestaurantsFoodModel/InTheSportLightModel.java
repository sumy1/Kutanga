...
public class InTheSportLightModel {
 ...
 private String ImagePath;
    ...
    public CopyOnWriteArrayList<inTheSportLightData> getInTheSport() {
        return inTheSport;
    }

    public void setInTheSport(CopyOnWriteArrayList<inTheSportLightData> inTheSport) {
        this.inTheSport = inTheSport;
    }

    private CopyOnWriteArrayList<inTheSportLightData> inTheSport;



    public InTheSportLightModel(JSONObject jsonObject){
        ...
        if(jsonObject.has(kOfferdata)){
            try {
                this.inTheSport = handleMarketPlace(jsonObject.getJSONArray(kOfferdata));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
    ...
    private CopyOnWriteArrayList<inTheSportLightData> handleMarketPlace(JSONArray jsonArray) throws JSONException {
        CopyOnWriteArrayList<inTheSportLightData> marketPlaces = new CopyOnWriteArrayList<>();
        for (int i = 0; i < jsonArray.length(); i++) {
            marketPlaces.add(new inTheSportLightData(jsonArray.getJSONObject(i)));
        }
        return marketPlaces;
    }
}
